...
def run_summary(cfg):
    ...
    linetab_outfname = os.path.join(summary_output_dir,
                                    f"{name}_{output_suffix}_lines.txt")
    abundtab_outfname = os.path.join(summary_output_dir,
                                     f"{name}_{output_suffix}_abunds.txt")
    if os.path.exists(linetab_outfname):
        print("Overwriting", linetab_outfname)
    if os.path.exists(abundtab_outfname):
        print("Overwriting", abundtab_outfname)
    ...
    if sumcfg.get("quick_no_errors", False):
        ...
        summarytab = get_fullerrors_summary(sumcfg, linetab)
        linetab, summarytab = get_fullerrors_limits(session, linetab,
                                                    summarytab)

    linetab["star"] = name
    summarytab["star"] = name
    linetab.write(linetab_outfname,
                  format="ascii.fixed_width_two_line",
                  overwrite=True)
    summarytab.write(abundtab_outfname,
                     format="ascii.fixed_width_two_line",
                     overwrite=True)
...
def get_quick_lines(session):
    from lesspayne.smh.spectral_models import (
        ProfileFittingModel,
    ...
    for i, model in enumerate(session.spectral_models):
        ...
        try:
            ...
            print("Exception:", e)
            logeps, staterr, e_Teff, e_logg, e_vt, e_MH, fwhm = (
                np.nan,
                np.nan,
                np.nan,
                np.nan,
        ...
        if isinstance(model, ProfileFittingModel):
            ...
            e_eqw = -999
        input_data = [
            i, wavelength, species, expot, loggf, logeps, eqw, e_eqw, fwhm
        ]
    ...
    for col in tab.colnames:
        if col in ["index", "wavelength", "species", "loggf", "star"]:
            continue
        tab[col].format = ".2f"
    return tab
...
def get_quick_summary(tab):
    ...
    cols = ["species", "elem", "N", "logeps", "sigma", "stderr", "[X/H]"]
    data = OrderedDict(zip(cols, [[] for col in cols]))
    for species in unique_species:
        ...
        input_data = [species, elem, N, logeps, stdev, stderr, XH]
...
def get_quick_limits(session, tab, summary_tab):
    ...
    for species in ul_species:
        if species in summary_tab["species"]:
            continue
        ...
        limit_XFe = limit_XFe2 if (species -
                                   int(species) > 0.01) else limit_XFe1
        input_data = [
            species,
            elem,
            N,
...
def get_fullerrors_lines(session,
                         minerr=0.001,
                         default_esys=0.1,
                         estimate_systematic_error=False):
    """
    ...
    for i, model in enumerate(session.spectral_models):
        ...
        try:
            ...
            sperrdict = model.metadata[
                "systematic_stellar_parameter_abundance_error"]
    ...
    if estimate_systematic_error:
        for species in np.unique(tab["species"]):
            ...
            # s_max = 2.
            s_max = (
                3.0  # Updated to a larger value because it was not always converging.
            )
            ...
            for i in range(100):
                ...
                if func(0) < func(s_max):
                    ...
                    break
                try:
                    s = optimize.brentq(func, 0, s_max, xtol=0.001)
...
def get_fullerrors_summary(sumcfg,
                           tab,
                           use_weighted_abundances_for_XH_XFe=True,
                           XFe_type=1):
    """
    Take a table of lines and turn them into standard abundance table
    """
    ...
    if sumcfg.get("add_systematic_uncertainties", False):
        additional_weights = sumcfg["systematic_uncertainties"]
    else:
    ...
    for species in unique_species:
        ...
        if finite.sum() != N:
            print("WARNING: species {:.1f} N={} != finite weights {}".format(
                species, N, finite.sum()))
        x = ttab["logeps"]
    ...
    if use_weighted_abundances_for_XH_XFe:
        delta_XY_0 = struct2array(
            np.array(summary_tab["e_Teff_w", "e_logg_w", "e_vt_w", "e_MH_w"]))
        var_X_stat = summary_tab["sigma_w"]**2 + summary_tab["stderr_w"]**2
    else:
        delta_XY_0 = struct2array(
            np.array(summary_tab["e_Teff", "e_logg", "e_vt", "e_MH"]))
    ...
    if len(summary_tab["[X/H]"]) > 0:
        ...
        if XFe_type == 1:
            ...
            ixion = np.array(
                [x - int(x) > 0.01 for x in summary_tab["species"]])
            summary_tab["[X/Fe]"] = summary_tab["[X/Fe1]"]
            summary_tab["e_XFe"] = summary_tab["e_XFe1"]
            summary_tab["[X/Fe]"][ixion] = summary_tab["[X/Fe2]"][ixion]
        ...
        for col in [
                "[X/Fe]", "[X/Fe1]", "[X/Fe2]", "e_XFe", "e_XFe1", "e_XFe2"
        ]:
            summary_tab.add_column(astropy.table.Column(np.zeros(0), col))
...
def get_fullerrors_limits(session, tab, summary_tab, XFe_type=1):
    ...
    cols = [
        ...
        "e_XH",
        "s_X",
    ] + ["[X/Fe1]", "e_XFe1", "[X/Fe2]", "e_XFe2", "[X/Fe]", "e_XFe"]
    assert len(cols) == len(summary_tab.colnames)
    data = OrderedDict(zip(cols, [[] for col in cols]))
    for species in ul_species:
        if species in summary_tab["species"]:
