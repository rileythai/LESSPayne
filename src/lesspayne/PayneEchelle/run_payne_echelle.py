...
def read_spectrum(fname):
    ...
    # print("vac:", wavelength[0][:10])

    # wavelength = vac_to_air(wavelength * u.Angstrom).value
    # print("vac to air performed!")
    # print("air:", wavelength[0][:10])
...
def run_payne_echelle(cfg):
    ...
    if NNtype == "default":
        Teff0, logg0, MH0, aFe0 = (
            pcfg["initial_parameters"]["Teff"],
            pcfg["initial_parameters"]["logg"],
            pcfg["initial_parameters"]["MH"],
            pcfg["initial_parameters"]["aFe"],
        )
        ...
        initial_stellar_labels = [
            Teff0, logg0, vt0, MH0, CFe0, MgFe0, CaFe0, TiFe0
        ]
    ...
    if pcfg["initial_velocity"] is None:
        ...
        print(
            f"Automatic velocity with template {vcfg['template_spectrum_fname']}"
        )
        template = Spectrum1D.read(vcfg["template_spectrum_fname"])

        # template._dispersion = air_to_vac(template._dispersion *
        #                                  u.Angstrom).value
        # print("template is now in VAC")

        rv_wave1, rv_wave2 = (
            float(vcfg["wavelength_region_min"]),
            float(vcfg["wavelength_region_max"]),
        )
        rv0, vhelcorr = quick_measure_mike_velocities(
            specfname,
            template_spectrum=template,
            wmin=rv_wave1,
            wmax=rv_wave2)
        print(
            f"Automatic velocity from {rv_wave1:.0f}-{rv_wave2:.0f}: {rv0:.1f} (+ {vhelcorr:.1f} for vhel)"
        )
    else:
    ...
    wavelength, spectrum, spectrum_err = utils.cut_wavelength(
        wavelength, spectrum, spectrum_err, wmin, wmax)
    ...
    spectrum_err[(spectrum_err == 0) | np.isnan(spectrum_err)] = 999.0
    spectrum_blaze = np.abs(spectrum_blaze)
    spectrum_blaze[spectrum_blaze == 0] = 1.0

    # rescale the spectra by its median so it has a more reasonable y-range
    spectrum, spectrum_err = utils.scale_spectrum_by_median(
        spectrum, spectrum_err.copy())
    spectrum_blaze = (spectrum_blaze /
                      np.nanmedian(spectrum_blaze, axis=1)[:, np.newaxis])
    ...
    wavelength, spectrum, spectrum_err, spectrum_blaze = (
        wavelength[~bad_orders],
        spectrum[~bad_orders],
        spectrum_err[~bad_orders],
        spectrum_blaze[~bad_orders],
    )
    ...
    spectrum_blaze, wavelength_blaze = utils.match_blaze_to_spectrum(
        wavelength, spectrum, wavelength_blaze, spectrum_blaze)

    norder, npix = wavelength.shape

    spectrum_err = utils.mask_wavelength_regions(wavelength, spectrum_err,
                                                 mask_list)
    ...
    mask = ((~np.isfinite(spectrum))
            | (~np.isfinite(spectrum_err))
            | (~np.isfinite(spectrum_blaze)))
    ...
    ## Set up RV Array
    RV_array = np.array([rv0 / 100.0])
    found = False
    for target_wavelength in rv_target_wavelengths:
        for iorder in range(num_order):
            if (wavelength[iorder, 0]
                    < target_wavelength) and (wavelength[iorder, -1]
                                              > target_wavelength):
                found = True
                break
        if found:
        ...
        raise RuntimeError(
            f"{specfname} does not have any of the target wavelengths: {rv_target_wavelengths}"
        )
    ...
    if NNtype == "default":
        ...
        errors_payne = utils.read_default_model_mask(
            wavelength_payne=model.wavelength_payne)
        model = DefaultPayneModel.load(
            NNpath,
            num_order=norder,
    ...
    print("starting fit")
    out = fitting.fit_global(
        spectrum,
        spectrum_err,
        spectrum_blaze,
        wavelength,
        model,
        initial_stellar_parameters=initial_stellar_labels,
        RV_array=RV_array,
        order_choice=[iorder],
        poly_coeff_min=poly_coeff_min,
        poly_coeff_max=poly_coeff_max,
        get_perr=True,
    )
    ...
    perr_print = np.abs(
        model.transform_coefficients(popt_best + perr_best) - popt_print)
    if NNtype == "default":
        print(
            "[Teff [K], logg, Fe/H, Alpha/Fe] = ",
            int(popt_print[0] * 1.0) / 1.0,
            int(popt_print[1] * 100.0) / 100.0,
            int(popt_print[2] * 100.0) / 100.0,
            int(popt_print[3] * 100.0) / 100.0,
        )
        print("vbroad [km/s] = ", int(popt_print[-2] * 10.0) / 10.0)
        print("RV [km/s] = ", int(popt_print[-1] * 10.0) / 10.0)
    elif NNtype == "yyli":
        print(
            "[Teff [K], logg, vt, Fe/H, CFe, MgFe, CaFe, TiFe] = ",
            int(popt_print[0] * 1.0) / 1.0,
            int(popt_print[1] * 100.0) / 100.0,
            int(popt_print[2] * 100.0) / 100.0,
            int(popt_print[3] * 100.0) / 100.0,
            int(popt_print[4] * 100.0) / 100.0,
            int(popt_print[5] * 100.0) / 100.0,
            int(popt_print[6] * 100.0) / 100.0,
            int(popt_print[7] * 100.0) / 100.0,
        )
        print("vbroad [km/s] = ", int(popt_print[-2] * 10.0) / 10.0)
        print("RV [km/s] = ", int(popt_print[-1] * 10.0) / 10.0)
    ...
    if pcfg["save_figures"]:
        plotting.save_figures_multipdf(
            name,
            wavelength,
            spectrum,
            spectrum_err,
